...
# Codex config: stows into $HOME, not ~/.config.
# Must run from outside ~/dotfiles, since .stowrc ignores `Codex` for `stow .`
cd ~ && stow --dir ~/dotfiles --target ~ Codex
...
### Codex (`Codex/`)
- Stow package targeting `$HOME` instead of `~/.config`, since Codex reads `~/.Codex`
- Tracks `settings.json`, `statusline.sh`, `agents/`, `skills/`, and the two plugin manifests
- Runtime state (plugin clones, transcripts, caches) stays untracked; see `Codex/README.md`
